...
fastify.post('/api/send', async (request, reply) => {
    ...
    try {
        ...
        // Ignore container ENV for multi-domain routing.
        const smtpPort = 587;
        ...
        // We must provide envelope when sending raw message, or let nodemailer parse it?
        // Nodemailer documentation says if 'raw' is provided, 'envelope' is recommended/required if not identifiable.
        // It's safer to provide envelope explicitly.
        // Recipient 'to' can be comma separated string, we need array? 
        // Nodemailer handles comma strings in 'to'.

        await smtpTransporter.sendMail({
            envelope: {
                from: sessionData.email,
                to: to // Nodemailer expects address string or array. 'to' variable is string.
            },
            raw: rawMessage
        });
...
fastify.get('/api/mails/:uid/download', async (request, reply) => {
    const { uid } = request.params as any;
    const { folder = 'INBOX' } = request.query as any;
    console.log(`📥 E-posta indirme isteği: UID=${uid}, Folder=${folder}`);
    const token = request.headers['authorization'];
...
const start = async () => {
    try {
        ...
        console.log(`🚀 Nerzen Webmail API ${port} portunda çalışıyor.`);
    } catch (err) {
